...
export const imageToDataFigure = (d) =>
	d.on('attribute:data-src:imageBlock', (e, data, { mapper, writer }) => {
...
export const imageDimensionsToDataFigure = (d) =>
	d.on('attribute:data-width:imageBlock', (e, data, { mapper, writer }) => {
		const figure = mapper.toViewElement(data.item);
		const img = figure.getChild(0);
